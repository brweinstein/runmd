...
fn find_all_code_blocks(content: &str) -> Vec<CodeBlock> {
    ...
    while i < lines.len() {
        ...
        if line.starts_with("```") && line.len() > 3 {
            ...
            // Skip if no language specified or invalid characters
            if language.is_empty() || !language.chars().all(|c| c.is_alphanumeric() || c == '_' || c == '-') {
                i += 1;
                continue;
            }
...
/// Process markdown by executing code blocks and attaching outputs with parallel execution.
pub async fn process_markdown(content: &str) -> Result<String> {
    ...
    // Use parallel processing for multiple blocks (>= 4), sequential for fewer
...
async fn process_markdown_sequential(content: &str, code_blocks: &[CodeBlock], languages: &Languages) -> Result<String> {
    if code_blocks.is_empty() {
...
async fn process_markdown_parallel(content: &str, code_blocks: &[CodeBlock], languages: &Languages) -> Result<String> {
    use futures::future::join_all;
    
    // Execute all code blocks in parallel
    let tasks: Vec<_> = code_blocks.iter().enumerate().map(|(i, block)| {
        let languages_clone = languages.clone();
        let block_clone = block.clone();
        
        async move {
            let timeout = if block_clone.code.len() > 1000 { 10 } else { 5 };
            let output = run_code(&block_clone.language, &block_clone.code, &languages_clone, timeout).await?;
